...
export const POST = async (req) => {//req is the request object
    ...
    try {
        ...
        const user = await Faculty.findOne({email:body.email}); // find the user by username form the database
        if(user){
            ...
            if(pass == decryptedPassword){// check if the password is correct
                let token = jwt.sign({user: user.user_name, email:user.email,role:"faculty"},process.env.JWT_SECRET);
